...
const Cart = () => {
  ...
  // Estado para controlar la visibilidad del botón de eliminación para cada ítem
...
useEffect(() => {
  ...
  const handleDelete = async (cartId) => {
    try {
      const VITE_BACKEND_HOST = import.meta.env.VITE_BACKEND_HOST;
      const response = await fetch(`${VITE_BACKEND_HOST}/cart/${cartId}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      const data = await response.json();
  ...
  const handleIncrement = async (cartId, quantity) => {
    try {
      ...
      const response = await fetch(`${VITE_BACKEND_HOST}/cart`, {
        ...
        credentials: 'include',
        body: JSON.stringify({ id: cartId, quantity: quantity }), // Incluir cartId y la nueva cantidad
      });
      const data = await response.json();
      if (response.ok) {
        fetchCart();
  ...
  const handleDecrement = async (cartId, quantity) => {
    try {
      ...
      const response = await fetch(`${VITE_BACKEND_HOST}/cart`, {
        ...
        credentials: 'include',
        body: JSON.stringify({ id: cartId, quantity: quantity }), // Incluir cartId y la nueva cantidad (negativa para decrementar)
      });
      const data = await response.json();
      if (response.ok) {
        fetchCart();
          ...
          <div key={item._id} className='card-container'>
            <div className='card'>
              ...
              >
                <div className="card-content-image" onClick={() => {
                  // Actualizar el estado para mostrar u ocultar el botón de eliminación del ítem actual
            ...
            {showDeleteButtons[item._id] && (
              <div className='delete-card'>
                <button onClick={() => handleDelete(item._id)}>    <svg xmlns="http://www.w3.org/2000/svg" width="38" height="43" viewBox="0 0 38 43" fill="none">
